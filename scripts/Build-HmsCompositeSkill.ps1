...
$ArtifactKind = 'hms-superpowers-composite'
...
function Get-ExactJunctionState {
    ...
    if ($linkType -ine 'Junction') {
        return [pscustomobject]@{ State = 'Conflict'; Detail = "Existing reparse point is not a Junction: $Link (LinkType='$linkType')" }
    }
    if (-not (Test-Path -LiteralPath $Target)) {
        return [pscustomobject]@{ State = 'Conflict'; Detail = "Expected Junction target is missing: $Target" }
    }
    ...
    foreach ($candidate in @($item.Target)) {
        ...
        try {
            if ((Get-CanonicalPath -Path ([string]$candidate)) -ieq $expected) {
                return [pscustomobject]@{ State = 'Exact'; Detail = "Junction targets $expected" }
            }
        }
        catch { }
    }
...
function Remove-ExactJunction {
    ...
    if ($boundary.State -ne 'Exact') {
        try { Restore-QuarantinedPath -Original $Link -Quarantine $quarantine }
        catch { throw "Discovery path changed at destructive boundary and rollback failed: $($_.Exception.Message)" }
        throw "Discovery path changed at destructive boundary; removal refused: $Link"
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        try { Restore-QuarantinedPath -Original $Link -Quarantine $quarantine }
        catch { throw "Junction removal failed with exit code $exitCode and rollback failed: $($_.Exception.Message)" }
        throw "Junction removal failed with exit code $exitCode; original path restored: $Link"
    }
}
...
function Assert-OwnedCompositeRoot {
    ...
    catch { throw "Refusing to replace composite directory with invalid ownership manifest: $Path" }
    if ([string]$manifest.managed_by -cne $ManagedBy -or [string]$manifest.artifact -cne $ArtifactKind) {
        throw "Refusing to replace composite directory with unexpected ownership: $Path"
    }
}
...
function Write-CompositeSkill {
    param(
        ...
        [Parameter(Mandatory)][string[]]$HmsReferences,
        [Parameter(Mandatory)][string[]]$SuperpowersReferences,
        [string]$TasteReference,
        [string]$ImpeccableReference
    )

    $enabled = @()
    foreach ($name in @('hms', 'superpowers', 'taste', 'impeccable')) {
        if ([bool]$Modules[$name]) { $enabled += $name }
    }
    ...
    $lines = @(
        ...
        '6. Load only the module references needed for the current task; internal MODULE.md files are references, not separately invokable Codex skills.',
        '',
...
$modules = [ordered]@{
    hms = [bool]$Hms
    superpowers = [bool]$Superpowers
    taste = [bool]$Taste
    impeccable = [bool]$Impeccable
}

if ($Hms -and -not (Test-Path -LiteralPath (Join-Path $InstallRoot 'skills\hms-superpowers\SKILL.md'))) {
    throw 'HMS Core is enabled but HMS skill sources are missing.'
}
if ($Superpowers -and -not (Test-Path -LiteralPath (Join-Path $SuperpowersRoot 'skills'))) {
    throw 'Superpowers is enabled but the pinned source repository is missing.'
}
...
foreach ($entry in $legacy) {
    $item = Get-Item -LiteralPath $entry.Link -Force -ErrorAction SilentlyContinue
...
if (Test-Path -LiteralPath $FinalRoot) {
    $compositeState = Get-ExactJunctionState -Link $CompositeLink -Target $FinalRoot
    if ($compositeState.State -eq 'Conflict') { throw "Composite discovery conflict: $($compositeState.Detail)" }
}
...
try {
    ...
    if ($Hms) {
        $hmsRefs = @(Copy-SkillCollection -SkillsDirectory (Join-Path $InstallRoot 'skills') -DestinationRoot (Join-Path $referencesRoot 'hms') -ModulePrefix 'references/hms')
    }
    if ($Superpowers) {
        $superRefs = @(Copy-SkillCollection -SkillsDirectory (Join-Path $SuperpowersRoot 'skills') -DestinationRoot (Join-Path $referencesRoot 'superpowers') -ModulePrefix 'references/superpowers')
    }
    if ($Taste) {
        $dest = Join-Path $referencesRoot 'taste'
        Copy-SkillModule -Source $tasteSource -Destination $dest
        $tasteRef = 'references/taste/MODULE.md'
    }
    if ($Impeccable) {
        $dest = Join-Path $referencesRoot 'impeccable'
        Copy-SkillModule -Source $impeccableSource -Destination $dest
        $impeccableRef = 'references/impeccable/MODULE.md'
    }
    ...
    $manifest = [ordered]@{
        ...
        composite_skill = $CompositeName
        generated_at_utc = [DateTime]::UtcNow.ToString('o')
        modules = $modules
        enabled_modules = $enabled
        source_heads = [ordered]@{
            hms = Get-GitHeadOrNull -Path $InstallRoot
            superpowers = Get-GitHeadOrNull -Path $SuperpowersRoot
            taste = Get-GitHeadOrNull -Path $TasteRoot
            impeccable = Get-GitHeadOrNull -Path $ImpeccableRoot
        }
        routing_contract = [ordered]@{
            governance = 'hms'
            engineering_method = 'superpowers'
            visual_direction = 'taste'
            ui_audit_polish = 'impeccable'
            concurrency = 'one-primary-owner-per-task-slice'
        }
    }
    ...
    try {
        ...
        $rollbackErrors = @()
        try {
            if ($createdComposite -and (Test-Path -LiteralPath $FinalRoot)) { Remove-ExactJunction -Link $CompositeLink -Target $FinalRoot }
        }
    ...
    Write-Host "PASS: compiled one Codex skill '$CompositeName' from enabled modules: $(if ($enabled.Count -eq 0) { 'none' } else { $enabled -join ', ' })."
    Write-Host "Composite bundle: $FinalRoot"
    if ($enabled.Count -gt 0) { Write-Host "Codex discovery: $CompositeLink" }
    else { Write-Host 'Codex discovery disabled because no modules are enabled.' }
}
finally {
